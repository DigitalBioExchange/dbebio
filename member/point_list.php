...
// 전체 데이터수 구하기
$sql_total = "select * from rb_point_history where mb_id = '".$member['mb_id']."' $search_query group by date_format(ph_regdate, '%Y-%m-%d') $order_query";
...
if($total){
	...
	$sql = "select * from rb_point_history where mb_id = '".$member['mb_id']."' $search_query group by date_format(ph_regdate, '%Y-%m-%d') $order_query $limit_query";
	$data = sql_list($sql);

	foreach ($data as $key => $value) {
		$sql_sub = "select * from rb_point_history where mb_id = '".$member['mb_id']."' and date_format(ph_regdate, '%Y-%m-%d') = '".substr($value['ph_regdate'], 0, 10)."' $order_query";

		$data_sub = sql_list($sql_sub);
		foreach ($data_sub as $key_2 => $value_2) {
			$data_sub[$key_2]['ph_memo'] = $value_2['ph_memo_'.$lang_code];
		}
