...
if($is_member){
	$_POST[od_name] = $member[mb_name];
	$_POST[od_tel] = $member[mb_tel];
	$_POST[od_zip] = $member[mb_zip];
	$_POST[od_addr1] = $member[mb_addr1];
	$_POST[od_addr2] = $member[mb_addr2];
}

if($is_member && $_POST[use_address] == '1' && $_POST[ad_idx]){
	$address = sql_fetch("select * from rb_address where mb_id = '".$member[mb_id]."' and ad_idx = '".$_POST[ad_idx]."'");
	$_POST[de_name] = $address[ad_name];
	$_POST[de_tel] = $address[ad_tel];
	$_POST[de_zip] = $address[ad_zip];
	$_POST[de_addr1] = $address[ad_addr1];
	$_POST[de_addr2] = $address[ad_addr2];
}
...
for($i=0;$i<$_POST[total_cnt];$i++){
	...
	//쿠폰사용처리
	if($cr_idx){
		sql_query("update rb_coupon_record set cr_status = 3, cr_use_date = now() where cr_idx = '$cr_idx'");
	}
}
...
alert("무통장입금 결제가 신청되었습니다..", "/shop/order_list.php");
//alert("무통장입금 결제가 신청되었습니다..", "/member/order_complete.php?m=$_POST[od_pd]&od_num=$_POST[od_num]");
